    ...
    public static void main(String[] args) {
        ...
        for (int trainingIterations : iterations) {
          ...
          for(double cooling : coolings) {
              ...
              results +=  "\nResults for SA:" + cooling + ": \nCorrectly classified " + correct + " instances." +
                        "\nIncorrectly classified " + incorrect + " instances.\nPercent correctly classified: "
                        + df.format(correct/(correct+incorrect)*100) + "%\nTraining time: " + df.format(trainingTime)
                        + " seconds\nTesting time: " + df.format(testingTime) + " seconds\n";

              final_result = oaNames[0] + "," + "iterations" + trainingIterations + "," + "coolings" + cooling + "," + "training accuracy" + "," + df.format(correct / (correct + incorrect) * 100)
                        + "," + "training time" + "," + df.format(trainingTime) + "," + "testing time" +
                        "," + df.format(testingTime);
              write_output_to_file("Optimization_Results", "abalone_SA.txt", final_result, true);
        }
