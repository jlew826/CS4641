...
clf = DecisionTreeClassifier(max_leaf_nodes = 10, min_samples_leaf = 20, max_depth= 5)
clf.fit(X_train, y_train)
prediction = clf.predict(X_test)
scores = cross_val_score(clf, X, y, cv=5)
...
# 
# min_samples_leaf = 20 (default is 1) in attempt to restrict the size of sample leaf
# 
...
clf = GradientBoostingClassifier(n_estimators = 300, max_depth = 1)
clf.fit(X_train, y_train)
prediction = clf.predict(X_test)
scores = cross_val_score(clf, X, y, cv=5)
...
# Pruning helped to avoid overfitting, therefore resulting in a better accuracy and cross validation score. I chose to prune by setting the following parameters:
# 
# n_estimators = 300 (default is 100) "The number of boosting stages to perform. Gradient boosting is fairly robust to over-fitting so a large number usually results in better performance."
# 
